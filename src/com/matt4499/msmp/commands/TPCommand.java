...
public class TPCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if(args.length < 1) {
            player.sendMessage(Main.hex("#dc143c&lSTAFF &f➠ #dc143cUsage: /tp <player>"));
            return true;
        }
        Player target = Bukkit.getPlayerExact(args[0]);
        if (target == null) {
            player.sendMessage(Main.hex("#dc143c&lSTAFF &f➠ #dc143cPlayer not found."));
            return true;
        }
        player.teleport(target);
        for(Player p : Bukkit.getOnlinePlayers()) {
            if(p.hasPermission("staff.tp")) {
                p.sendMessage(Main.hex("#dc143c&lSTAFF &f➠ #dc143c" + player.getDisplayName() + " &7has teleported to #dc143c"+target.getDisplayName()));
            }
        }
