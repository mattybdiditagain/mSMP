...
public class HomeCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if (args.length == 0) {
            p.sendMessage(Main.hex("#dc143c&lHOMES &f➠ #dc143cUsage: /home <name>"));
            return true;
        }
        String homeName = args[0];
        Location loc = homeHelper.getPlayerHomeLocation(p, homeName);
        if (loc == null) {
            p.sendMessage(Main.hex("#dc143c&lHOMES &f➠ #dc143cHome " + homeName + " not found!"));
            return true;
        }
        BackCommand.back.put(p, p.getLocation());
        p.teleport(loc);
        p.sendMessage(Main.hex("#dc143c&lHOMES &f➠ &7You teleported to home: #dc143c" + homeName));
