...
public class CCBanCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if(args.length == 0) {
            admin.sendMessage(Main.hex("#dc143c&lSTAFF &f➠ #dc143cUsage: /ccban <player>"));
            return true;
        }
        Player target = Bukkit.getPlayerExact(args[0]);
        if(target == null) {
            admin.sendMessage(Main.hex("#dc143c&lSTAFF &f➠ #dc143cPlayer not found."));
            return true;
        }
        ...
        Bukkit.broadcastMessage(Main.hex("#dc143c&lSTAFF &f➠ #dc143cChat cleared by " + admin.getDisplayName() + "."));
